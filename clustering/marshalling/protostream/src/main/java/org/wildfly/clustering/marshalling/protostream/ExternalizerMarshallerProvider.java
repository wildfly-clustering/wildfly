 ...
 * @author Paul Ferraro
 */
public class ExternalizerMarshallerProvider implements SerializationContext.MarshallerProvider {
    ...
    public <E extends Externalizer<?>> ExternalizerMarshallerProvider(Iterable<E> externalizers) {
        for (Externalizer<?> externalizer : externalizers) {
            Class<?> targetClass = externalizer.getTargetClass();
            BaseMarshaller<?> marshaller = targetClass.isEnum() ? new EnumMarshaller<>(targetClass.asSubclass(Enum.class)) : new ExternalizerMarshaller<>(externalizer);
