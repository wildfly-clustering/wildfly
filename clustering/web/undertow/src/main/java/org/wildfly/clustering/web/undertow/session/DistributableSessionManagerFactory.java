...
public class DistributableSessionManagerFactory implements io.undertow.servlet.api.SessionManagerFactory {
    ...
    public io.undertow.server.session.SessionManager createSessionManager(final Deployment deployment) {
        ...
        io.undertow.server.session.SessionManager result = new DistributableSessionManager(info.getDeploymentName(), manager, listeners, statistics);
        Integer defaultSessionTimeout = this.config.getDefaultSessionTimeout();
        if (defaultSessionTimeout != null) {
            result.setDefaultSessionTimeout(defaultSessionTimeout.intValue());
        }
        return result;
    }
}
