...
routing::
This defines the algorithm for generating the route to be appended onto the session ID (within the JSESSIONID cookie, or when encoding URLs).
...
This is only generated when creating the JSESSIONID cookie, or when encoding URLs via `HttpServletResponse.encodeURL()` and `encodeRedirectURL()`.
Possible values are:
NONE:::
Disables route annotation of session ID.
LOCAL:::
Routes subsequent requests to the node that last handled a request for a given session.
OWNER:::
Routes subsequent requests to the primary owner of a given session.
Behaves the same as LOCAL if cache is not distributed nor replicated.
...
To use an existing session management profile, a web application should include a distributable-web.xml deployment descriptor located within the application's /WEB-INF directory.
...
Alternatively, the same XML can be defined within an existing jboss-all.xml deployment descriptor:
...
<?xml version="1.0" encoding="UTF-8"?>
<distributable-web xmlns="urn:jboss:distributable-web:1.0">
    <infinispan-session-management cache-container="web" cache="foo" granularity="ATTRIBUTE" routing="LOCAL"/>
</distributable-web>
----
